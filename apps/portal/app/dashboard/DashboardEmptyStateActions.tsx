  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs">
        <Link href="/dashboard/settings" className="underline text-muted-foreground hover:text-foreground">Confirm service area</Link>
        {bookingLink ? (
          <button type="button" onClick={copyBookingLink} className="underline text-muted-foreground hover:text-foreground">
            {copied ? "Booking link copied" : "Share booking link"}
          </button>
        ) : null}
        <Link href="/dashboard/crm" className="underline text-muted-foreground hover:text-foreground">View requests</Link>
        <Link href="/dashboard/actions" className="underline text-muted-foreground hover:text-foreground">Open Action Inbox</Link>
      </div>
    </div>
  );
}
