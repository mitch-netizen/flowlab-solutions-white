  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs">
        {bookingLink ? (
          <button type="button" onClick={copyBookingLink} className="text-muted-foreground hover:text-foreground">
            {copied ? "Booking link copied" : "Copy booking link"}
          </button>
        ) : null}
        <Link href="/dashboard/crm" className="text-muted-foreground hover:text-foreground">View requests</Link>
        <Link href="/dashboard/settings" className="text-muted-foreground hover:text-foreground">Edit settings</Link>
      </div>
    </div>
  );
}
