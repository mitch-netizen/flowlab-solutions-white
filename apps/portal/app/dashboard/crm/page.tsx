...
export default async function CrmPage({
  ...
  return (
      ...
      <form action="/api/tenant/crm/customers" method="post" className="rounded-lg border bg-card p-4 space-y-4">
        <div className="space-y-2">
          <div className="eyebrow">Create customer</div>
          <h2>Add a customer manually</h2>
          <p className="text-sm text-muted-foreground">No inbound requests yet? Create a customer here so you can start preparing quotes right away.</p>
        </div>
        <div className="grid gap-3 md:grid-cols-3">
          <input className="w-full rounded-lg border bg-background px-3 py-2 text-sm" name="firstName" placeholder="First name" required />
          <input className="w-full rounded-lg border bg-background px-3 py-2 text-sm" name="lastName" placeholder="Last name" required />
          <input className="w-full rounded-lg border bg-background px-3 py-2 text-sm" name="email" placeholder="Email" type="email" required />
        </div>
        <div className="grid gap-3 md:grid-cols-2">
          <input className="w-full rounded-lg border bg-background px-3 py-2 text-sm" name="phone" placeholder="Phone (optional)" />
          <input className="w-full rounded-lg border bg-background px-3 py-2 text-sm" name="suburb" placeholder="Suburb (optional)" />
        </div>
        ...
        <div className="space-y-3">
          {snapshot.enquiries.length > 0 ? (
            ...
            <div className="space-y-3">
              ...
              <p className="text-sm text-muted-foreground">Need to start quoting now? Use the &quot;Create customer&quot; form above.</p>
