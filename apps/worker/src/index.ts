...
/** Fetch and decrypt credentials for a given service, returning empty object if not configured. */
...
async function processJob(job: Awaited<ReturnType<typeof claimPendingAutomationJobs>>[number]) {
  ...
  switch (job.kind) {
    ...
    case "quote.accepted": {
      ...
      // SMS customer if phone available
      if (customer?.phone) {
        try {
    ...
    case "invoice.created": {
      ...
      // SMS customer with payment link
      if (customer?.phone) {
        try {
    ...
    case "retention.feedback_request": {
      ...
      if (customer?.phone) {
        try {
          ...
          // Feedback can be submitted via the enquiry portal — a simple star rating flow
          await sendSms(
            twilioCredentials,
            customer.phone,
    ...
    case "schedule.recalculate": {
      ...
      if (flagged.length > 0) {
        const makeResult = await fireMakeWebhook(makeCredentials, "scheduleUpdateWebhookUrl", {
          ...
          recommendations: flagged.map((r) => ({ severity: r.severity, summary: r.summary }))
        });
    ...
    case "learning.weekly_analysis": {
      const [tenant] = await Promise.all([getTenantWithProfile(tenantId)]);

      // Pull last 30 completed jobs with time estimate history
      ...
      if (!pricingRate) break;

      // Build a prompt asking Claude to analyse the variance and suggest rate adjustments
      const avgVariance = history.reduce((sum, h) => sum + h.variancePct, 0) / history.length;
      const overEstimated = history.filter((h) => h.variancePct < -15).length;
      const underEstimated = history.filter((h) => h.variancePct > 15).length;
...
Return an empty array [] if no changes are recommended. Return ONLY JSON, no markdown.`
      ...
      await logPlatformEvent({
        ...
        service: "claude",
        direction: "outbound",
        status: suggestions.length > 0 ? "success" : "success",
        durationMs,
...
async function run() {
  ...
  const jobs = await claimPendingAutomationJobs(25);
  ...
  console.log(`Processed ${jobs.length} automation job(s).`);
}
